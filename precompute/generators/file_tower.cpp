...
int main() {
  ...
  freopen("file_tower_magic_number.txt", "w", stdout);
  ...
  */
  ...
  for(int i=0;i<64;i++) {
    ...
    uint64_t mask = tower_pos;

    // << 1
    uint64_t curr_pos = tower_pos;
    ...
    curr_pos = tower_pos;
    // << 8
    while((curr_pos&ROW8) == 0 && curr_pos != 0) {
      curr_pos <<= 8;
      mask |= curr_pos;
    }
    ...
    curr_pos = tower_pos;
    // >> 8
    while((curr_pos&ROW1) == 0 && curr_pos != 0) {
      curr_pos >>= 8;
      mask |= curr_pos;
    }
  ...
  for(int i=0;i<masks.size();i++) {
    ...
    uint64_t mask_no_center = (mask & ~center & ~ROW1 & ~ROW8);

    int bits_on = __builtin_popcountll(mask_no_center);
    vector<int> bit_pos;
    
    for(int b=0;b<64;b++) {
      if((mask_no_center&(uint64_t(1)<<b)) != 0) bit_pos.push_back(b);
    }
