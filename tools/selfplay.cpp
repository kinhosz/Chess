...
//
// usage: selfplay [depth] [max_moves] [--quiet-profiler]

int main(int argc, char** argv) {
  if(argc > 1 && (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help")) {
    std::cerr << "usage: " << argv[0] << " [depth=5] [max_moves=40] [--quiet-profiler]\n";
    return 0;
  }

  int depth = argc > 1 ? std::atoi(argv[1]) : 5;
  int max_moves = argc > 2 ? std::atoi(argv[2]) : 40;
  bool quiet_profiler = argc > 3 && std::string(argv[3]) == "--quiet-profiler";
  ...
  for(; m < max_moves; m++) {
    ...
    i5 mv = engine.getNextMove(depth, &nodes);
